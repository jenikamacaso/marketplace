...
app.post('/register', function (req, res) {
    ...
    db.serialize(() => {
        db.run('INSERT INTO users(name, email, password, role) VALUES(?,?,?,?)', [req.body.name, req.body.email, req.body.password, req.body.role], function (err) {
            if (err) {
                console.log(err.message);
                io.emit('result', 'An error occurred');
            }
            console.log("New User has been added");
            io.emit('result', 'User added successfully');
        });
...
app.post('/login', function (req, res) {
    db.serialize(() => {
        db.each('SELECT * FROM users WHERE email =?', [req.body.email], function (err, row) {
            if (err) {
                io.emit('result', 'An error occurred');
                console.error(err.message);
            }
...
app.post('/message', function (req, res) {
    db.serialize(() => {
        db.run('INSERT INTO msg(name,phone,email,message) VALUES(?,?,?,?)', [req.body.name, req.body.phone, req.body.email, req.body.message], function (err) {
            if (err) {
                console.log(err.message);
                io.emit('result', 'An error occurred');
            }
